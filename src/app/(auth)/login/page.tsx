"use client";

import { useState } from "react";
import PageLayout from "@/components/layout/PageLayout";
import { Card } from "@/components/ui/Card";
import { Button } from "@/components/ui/Button";
import { Input } from "@/components/ui/Input";
import { Tabs, TabList, Tab, TabPanel } from "@/components/ui/Tabs";

export default function LoginPage() {
  const [email, setEmail] = useState("");
  const [phone, setPhone] = useState("");
  const [sent, setSent] = useState(false);

  return (
    <PageLayout showWallpaper>
      <div className="flex items-center justify-center min-h-screen px-4 py-8">
        <Card frosted elevation="xl" padding="lg" className="w-full max-w-sm">
          <div className="text-center mb-6">
            <h1 className="text-h2 font-bold">Welcome back</h1>
            <p className="text-body-sm text-[var(--text-secondary)] mt-1">
              Sign in to manage your transfers
            </p>
          </div>

          {sent ? (
            <div className="text-center py-8">
              <div className="w-16 h-16 rounded-full bg-green-50 dark:bg-green-900/20 flex items-center justify-center mx-auto mb-4">
                <svg className="w-8 h-8 text-nigerian-green" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                </svg>
              </div>
              <h2 className="text-h3 font-bold mb-2">Check your inbox</h2>
              <p className="text-body-sm text-[var(--text-secondary)]">
                We sent a sign-in link to your email. Click it to continue.
              </p>
            </div>
          ) : (
            <>
              <Tabs defaultTab="email">
                <TabList>
                  <Tab id="email">Email</Tab>
                  <Tab id="phone">Phone</Tab>
                  <Tab id="google">Google</Tab>
                </TabList>

                <TabPanel id="email">
                  <form onSubmit={(e) => { e.preventDefault(); setSent(true); }} className="space-y-4">
                    <Input label="Email address" type="email" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} required />
                    <Button variant="primary" size="lg" fullWidth type="submit">Send magic link</Button>
                  </form>
                </TabPanel>

                <TabPanel id="phone">
                  <form onSubmit={(e) => { e.preventDefault(); setSent(true); }} className="space-y-4">
                    <Input label="Phone number" type="tel" placeholder="+234 800 000 0000" value={phone} onChange={(e) => setPhone(e.target.value)} required />
                    <Button variant="primary" size="lg" fullWidth type="submit">Send OTP</Button>
                  </form>
                </TabPanel>

                <TabPanel id="google">
                  <Button variant="outline" size="lg" fullWidth className="mt-2">
                    <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24" aria-hidden="true">
                      <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/>
                      <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                      <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
                      <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
                    </svg>
                    Continue with Google
                  </Button>
                </TabPanel>
              </Tabs>

              <p className="text-center text-caption-style text-[var(--text-muted)] mt-6">
                No account needed to send files.{" "}
                <a href="/" className="text-nigerian-green hover:underline">Send files now</a>
              </p>
            </>
          )}
        </Card>
      </div>
    </PageLayout>
  );
}
